...
yearly_representation <- data_gamm_prep %>%
  mutate(upload_year = year(dateUploaded)) %>%
  group_by(upload_year, dialect) %>%
  summarize(total_datasets = n(), .groups = "drop") %>%
  # Set minimum threshold for a "fair" representation (e.g., 20 datasets)
  filter(total_datasets >= 20) %>%
  group_by(dialect) %>%
  summarize(first_stable_year = min(upload_year))

data_gamm_prep <- data_gamm_prep %>% 
  filter(year(dateUploaded) > 2014) %>% 
  #filter(dialect == "EML") %>% 
  mutate(
  ...
  mutate(
    date_parsed = as_datetime(dateUploaded),
    upload_year_dec = decimal_date(date_parsed),
  ) 
```


```{r}
gam_F <- bam(scoreFindable ~ s(upload_year_dec, by = dialect, k = 5) + dialect + s(datasource, bs = "re"),,
             discrete = TRUE, nthreads = 10, method = "fREML", data = data_gamm_prep)

gam_A <- bam(scoreAccessible ~ s(upload_year_dec, by = dialect, k = 5) + dialect + s(datasource, bs = "re"),,
             discrete = TRUE, nthreads = 10, method = "fREML", data = data_gamm_prep)

gam_I <- bam(scoreInteroperable ~ s(upload_year_dec, by = dialect, k = 5) + dialect + s(datasource, bs = "re"), ,
             discrete = TRUE, nthreads = 10, method = "fREML", data = data_gamm_prep)

gam_R <- bam(scoreReusable ~ s(upload_year_dec, by = dialect, k = 5) + dialect + s(datasource, bs = "re"), ,
             discrete = TRUE, nthreads = 10, method = "fREML", data = data_gamm_prep)


est_F <- smooth_estimates(gam_F) %>% mutate(Metric = "Findable")
est_A <- smooth_estimates(gam_A) %>% mutate(Metric = "Accessible")
est_I <- smooth_estimates(gam_I) %>% mutate(Metric = "Interoperable")
est_R <- smooth_estimates(gam_R) %>% mutate(Metric = "Reusable")

all_fair_curves <- bind_rows(est_F, est_A, est_I, est_R) %>%
  filter(grepl("upload_year_dec", .smooth)) %>%
  mutate(Metric = factor(Metric, levels = c("Findable", "Accessible", "Interoperable", "Reusable")),
         .lower_ci = .estimate - (1.96 * .se),
         .upper_ci = .estimate + (1.96 * .se))

ggplot(all_fair_curves, aes(x = upload_year_dec, y = .estimate, color = dialect, fill = dialect)) +
  geom_ribbon(aes(ymin = .lower_ci, ymax = .upper_ci), alpha = 0.15, color = NA) +
  geom_line(linewidth = 1.2) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", linewidth = 0.5, alpha = 0.6) +
  geom_rug(
    data = data_gamm_prep, 
    aes(x = upload_year_dec, color = dialect), 
    inherit.aes = FALSE, 
    sides = "b", 
    alpha = 0.05, # Highly transparent since you have so many datasets!
    length = unit(0.04, "npc") # Makes the ticks slightly shorter so they don't crowd the lines
  ) +
  facet_wrap(~ Metric, scales = "free_y", ncol = 2) +
  #scale_color_manual(values = c("Findable" = "#c70a61", "Accessible" = "#ff582d", "Interoperable" = "#1a6379", "Reusable" = "#60c5e4")) +
  #scale_fill_manual(values = c("Findable" = "#c70a61", "Accessible" = "#ff582d", "Interoperable" = "#1a6379", "Reusable" = "#60c5e4")) +
  scale_color_manual(
    values = c(
      "EML" = "#26547c", 
      "ISO" = "#ef476f", 
  ...
  theme(
    strip.background = element_rect(fill = "#2C3E50"), # Dark header boxes for the F, A, I, R titles
  ...
  labs(
    title = "Evolution of FAIR Scores by Metadata Dialect (2010 - Present)",
    x = "Upload Year",
    y = "Effect on Score (%)",
    color = "Metadata Dialect",
    fill = "Metadata Dialect"
  )
```
...
ggsave("gamm_results.png", width = 20, height = 10, units = "in")
```



```{r}
gam_F <- bam(scoreFindable ~ s(upload_year_dec, by = dialect) + dialect + s(datasource, bs = "re"),
             discrete = TRUE, nthreads = 10, method = "fREML", data = data_gamm_prep)

gam_A <- bam(scoreAccessible ~ s(upload_year_dec, by = dialect) + dialect + s(datasource, bs = "re"),
             discrete = TRUE, nthreads = 10, method = "fREML", data = data_gamm_prep)

gam_I <- bam(scoreInteroperable ~ s(upload_year_dec, by = dialect) + dialect + s(datasource, bs = "re"),
             discrete = TRUE, nthreads = 10, method = "fREML", data = data_gamm_prep)

gam_R <- bam(scoreReusable ~ s(upload_year_dec, by = dialect) + dialect + s(datasource, bs = "re"),
             discrete = TRUE, nthreads = 10, method = "fREML", data = data_gamm_prep)

gam_overall <- bam(scoreOverall ~ s(upload_year_dec, by = dialect) + dialect + s(datasource, bs = "re"),
                   discrete = TRUE, nthreads = 10, method = "fREML", data = data_gamm_prep)
...
ggplot(all_fair_curves, aes(x = upload_year_dec, y = .estimate, color = dialect, fill = dialect)) +
  ...
  facet_wrap(~ Metric, scales = "free_y", ncol = 2) +
  scale_color_manual(values = c("EML" = "#D55E00", "ISO" = "#0072B2", "SOSO" = "#009E73")) +
  scale_fill_manual(values = c("EML" = "#D55E00", "ISO" = "#0072B2", "SOSO" = "#009E73")) +
  theme_bw(base_size = 14) +
  theme(
    strip.background = element_rect(fill = "#2C3E50"), # Dark header boxes for the F, A, I, R titles
    strip.text = element_text(color = "white", face = "bold", size = 12),
...
data_change_avg <- data_ver %>% 
  pivot_longer(cols = starts_with("score"), names_to = "metric", values_to = "mean") %>% 
  mutate(datasource = gsub("urn:node:", "", datasource)) %>% 
  mutate(mean = mean*100) %>% 
  select(sequenceId, datasource, metric, version_position, mean) %>%
  pivot_wider(names_from = version_position, values_from = mean) %>% 
  mutate(version_change = last-first) %>% 
  group_by(datasource, metric) %>%
  summarise(
    mean_first = mean(first, na.rm = TRUE),
    mean_last  = mean(last, na.rm = TRUE),
    mean_change = mean(version_change, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(score_change = case_when(
    mean_last > mean_first ~ "Increased",
    mean_last < mean_first ~ "Decreased",
    TRUE                   ~ "No Change")
  )

data_plot <- data_change_avg %>%
  pivot_longer(
    cols = c(mean_first, mean_last),
    names_to = "version_position", 
    values_to = "plot_mean"
  ) %>%
  mutate(
    version_position = gsub("mean_", "", version_position),
    datasource = forcats::fct_reorder(datasource, mean_change) 
  ) %>% 
  filter(metric != "scoreOverall")

```

```{r}
ggplot(data_plot, aes(x = plot_mean, y = datasource)) +
