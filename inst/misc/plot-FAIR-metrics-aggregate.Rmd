...
node_status <- read.csv("https://raw.githubusercontent.com/datadavev/mnstatus/refs/heads/main/data/node_status.csv") %>% 
    filter(as.Date(cn.latest) > as.Date("2023-01-01"))
```
...
data_recent <- data %>% filter(datasource %in% node_status$node_id)
```
...
```{r}
ggplot(t, aes(x = ym, y = mean, color = metric)) +
    geom_point() +
    facet_wrap(~datasource, ncol = 3) +
    theme_bw()


ggsave("all_nodes_timeline_pts.png", width = 6, height = 30, units = "in")
...
```{r}
data_clean <- assign_sequence_id(data)
data_ver <- get_first_last_versions(data_clean)
```
...
```{r}
target_nodes <- c("urn:node:ARCTIC", "urn:node:KNB", "urn:node:ESS_DIVE", "urn:node:EDI", "urn:node:LTER", "urn:node:PISCO")
...
plot_data <- version_timelines %>%
    filter(version_position %in% c("first", "last"))
...
p <- ggplot(
    ...
    facet_wrap(~datasource, ncol = 2, scales = "fixed") +
...
data_change_avg <- data_ver %>% 
    ...
    filter(n > 100) %>% 
    filter(!grepl("CANWIN", datasource)) %>% 
...
```{r}
ggsave("FAIR-inital-final-mean.png", width = 6, height = 8, units = "in")
```
...
df <- data %>% 
    filter(is.na(obsoletedBy)) %>% 
    filter(dateUploaded > as.Date("2015-01-01")) %>% 
    ...
    split(.$dialect) %>% 
    purrr::map_dfr(~ get_scores_mean_ym(.x), .id = "dialect")
...
get_variance <- function(target_metric, data) {
  ...
  df_subset <- data %>% 
    ...
    mutate(time_numeric = as.numeric(as.factor(ym)))
  
  fit <- lm(score_100 ~ time_numeric + dialect, data = df_subset)
...
```{r}
df %>% 
    filter(metric %in% c("f", "a", "i", "r")) %>% 
ggplot(aes(x = ym, y = score_100, color = metric)) +
    geom_point() +
    geom_smooth() +
    facet_wrap(~dialect)
```
