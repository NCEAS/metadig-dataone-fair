...
# figure out what year we can start and get reasonable ISO representation
...
Experimenting with k...there is so much noise in the data it is impossible to overfit. The wildly variable ISO records don't help things either. Look at k ranging from 5 - 500.

```{r}
k <- 10

gam_F <- bam(scoreFindable ~ s(upload_year_dec, by = dialect, k = k) + dialect + s(datasource, bs = "re"),,
             discrete = TRUE, nthreads = 10, method = "fREML", data = data_gamm_prep)

gam_A <- bam(scoreAccessible ~ s(upload_year_dec, by = dialect, k = k) + dialect + s(datasource, bs = "re"),,
             discrete = TRUE, nthreads = 10, method = "fREML", data = data_gamm_prep)

gam_I <- bam(scoreInteroperable ~ s(upload_year_dec, by = dialect, k = k) + dialect + s(datasource, bs = "re"), ,
             discrete = TRUE, nthreads = 10, method = "fREML", data = data_gamm_prep)

gam_R <- bam(scoreReusable ~ s(upload_year_dec, by = dialect, k = k) + dialect + s(datasource, bs = "re"), ,
             discrete = TRUE, nthreads = 10, method = "fREML", data = data_gamm_prep)
...
dialect_colors <- c("EML" = "#26547c", "ISO" = "#ef476f", "SOSO" = "#ffd166")

pc <- ggplot(all_fair_curves, aes(x = upload_year_dec, y = .estimate, color = dialect, fill = dialect)) +
  ...
  scale_color_manual(values = dialect_colors) +
  scale_fill_manual(values = dialect_colors) +
...
p_lanes <- ggplot(data_gamm_prep, aes(x = upload_year_dec, y = dialect, color = dialect)) +
  geom_point(shape = 124, size = 6, alpha = 0.5) + 
  scale_color_manual(values = dialect_colors) +
  theme_bw(base_size = 14) +
  theme(
...
final_plot <- pc / p_lanes + plot_layout(heights = c(8, 1))
ggsave("gamm_results_dialect.png", final_plot, height = 15, width = 8)
```
...
df <- data %>% 
  ...
  mutate(year = year(dateUploaded)) %>%
  select(year, datasource, dialect, formatId, pid)
  group_by(year, dialect) %>% 
  summarise(n = n())

```
```{r}
ggplot(df, aes(x = year, y = n, color = dialect)) + geom_line() + scale_y_log10(labels = scales::label_log())
```
