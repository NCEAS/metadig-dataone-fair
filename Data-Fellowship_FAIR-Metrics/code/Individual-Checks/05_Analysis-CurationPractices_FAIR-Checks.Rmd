...
plot5b <- ggplot(plotData_5_post2016, aes(x=mean, y=check_name %>% forcats::fct_reorder(-mean))) +
  ...
  scale_shape_manual(values=shapeValues,
                     name="FAIR Score at",
                     labels=c("pre-ADC", "final publication (ADC-wide)", "initial submission (ADC-wide)")) +
  scale_fill_manual(values=fillValues,
                    name="FAIR Score at",
                    labels=c("pre-ADC", "final publication (ADC-wide)", "initial submission (ADC-wide)")) +
  scale_color_manual(values=colorValues,
...
plot5b <- plot5b + plot_annotation(
  title = 'Proportion of metadata meeting individual FAIR checks',
  subtitle = 'for the Arctic Data Center and pre-ADC data',
  caption = '2020-10-08'
)
```
...
```{r, eval=FALSE}
#save plot as PNG to aurora
ggsave(filename="Figure-05b_2020-10-15_FAIR-IndividualChecksOverTime_withACADIS.jpg",
       path=here("figures"),
       dpi = 400,
       units="in",
       width=13,
