...
make_flower_plot <- function(.Data, title = NA) {
  ...
  plot_obj <- ggplot2::ggplot(data = .Data, ggplot2::aes(x = pos, y = score_100, fill = label, width = weight)) +
    ...
    ggplot2::geom_bar(ggplot2::aes(y = 100), stat = "identity", color = light_line, fill = white_fill, linewidth = 0.2) + 
    ggplot2::geom_errorbar(ggplot2::aes(ymin = 100, ymax = 100), linewidth = 0.5, color = light_line) +
    ggplot2::geom_bar(stat = "identity", color = dark_line, linewidth = 0.2) + 
    ggplot2::geom_errorbar(ggplot2::aes(ymin = score_100, ymax = score_100), linewidth = 0.5, color = dark_line) + 
    ggplot2::geom_errorbar(ggplot2::aes(ymin = 0, ymax = 0), linewidth = 0.5, color = dark_line) + 
    
    ggplot2::coord_polar(start = 0) + 
    ggplot2::scale_x_continuous(breaks = .Data$pos, limits = p_limits) + 
    ggplot2::scale_y_continuous(limits = c(-blank_circle_rad, 110), breaks = c(0, 25, 50, 75, 100)) +
  ...
  plot_obj <- plot_obj + ggplot2::theme_void() + 
    ggplot2::theme(
      ...
      panel.grid.major.y = ggplot2::element_line(color = "grey85", linewidth = 0.3, linetype = "dashed")
    )
  
  # Inner score percentages on the petals themselves
  #plot_obj <- plot_obj + ggplot2::geom_text(
  #    ggplot2::aes(label = round(score_100), x = pos), 
