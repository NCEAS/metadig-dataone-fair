...
```{r flower_cumulative_by_dialect}
cum_dialect <- score_dialect %>% filter(metric %in% c("Cum. Findable", "Cum. Accessible", "Cum. Interoperable", "Cum. Reusable"))

cum_dialect %>%
  split(.$dialect) %>% 
  purrr::iwalk( ~fair_flower(df = .x, title = .y, filename = paste0("dataone-fair-flower-", .y, ".png") ))
```
...
ggsave("dataone-fair-scores-repos.png", width = 8, height = 10)
p
```
...
p <- ggplot(data=repo_monthly, mapping=aes(x=ym, y=mean, color=metric)) +
  ...
  ylab("Average FAIR Score (%)") +
  ggtitle(paste0("DataONE: FAIR scores for selected repositories"))
ggsave("dataone-fair-scores-repos-monthly.png", width = 8, height = 15)
p
```
...
p <- ggplot(data=repo_monthly_selected, mapping=aes(x=ym, y=mean, color=metric)) +
  ...
  ylab("Average FAIR Score (%)") +
  ggtitle(paste0("DataONE: FAIR scores for selected repositories"))
ggsave("dataone-fair-scores-repos-monthly-zoom.png", width = 8, height = 8)
p
```
