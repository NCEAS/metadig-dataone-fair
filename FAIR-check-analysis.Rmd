...
load_check_data <- function() {
  ...
  checks_joined_file <- paste0(check_data, "/fair-2.1-checks-joined.rda")
  runs_file <- paste0(check_data, "/fair-2.1-runs.rda")
...
runs_temp_file <- paste0(check_data, "/fair-2.1-runs-temp.rda")
if (file.exists(runs_temp_file)) {
  loaded <- load(runs_temp_file)
}
ncei_runs <- runs_temp %>% 
  filter(origin_mn %in% c("urn:node:NCEI"), ser_version > 1)

ncei_checks_current <- ncei_runs %>% 
  filter(is.na(obsoleted_by), !is.na(ser_version)) %>% 
  inner_join(checks, by = "run_id") %>% 
  arrange(check_type)
```
...
ggplot(ncei_checks_current, aes(x=forcats::fct_rev(check_name), fill=check_status)) + 
  geom_bar(position="fill") +
  theme_bw() +
  #theme(title=element_text(paste0("NCEI, subsample current versions, ", nrow(ncei_runs)))) +
  scale_fill_manual(values = status_palette) +
  facet_wrap(facets = ~ check_type, ncol = 4) +
  xlab("Check Name") +
  ylab("Proportion") +
  coord_flip()
ggsave("figures/ncei-fair-sample.png", height=10, width=10)
```
